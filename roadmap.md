...
- [ ] Rebuild dashboard visual to match uploaded Tameeni Care reference: light theme, "Pages · live traffic" sidebar with All pages/Quote/Insurer selected/Payment counters, sessions table with Session/Customer/Insurer/Stage/Updated/Action, session modal with Customer + Submissions two-column + Step actions grid + Redirect customer pills + Block session
- [ ] Tighten "live" filter to only visitors seen in the last few minutes so old rows aren't misread as active
