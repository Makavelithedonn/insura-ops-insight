...
import {
  ...
  Star, ArrowLeft, Sparkles, TrendingDown, Clock, Shield,
  Award, Users2, Building2,
} from "lucide-react";
import { insuranceTypes, insuranceCompanies, testimonials, features, carBrands } from "@/lib/insurance-data";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "قارن عروض تأمين السيارات من أكثر من 20 شركة تأمين. إصدار فوري وربط مباشر بنجم." },
      { property: "og:title", content: "بيكير — مقارنة تأمين السيارات" },
      { property: "og:description", content: "قارن عروض تأمين السيارات من أكثر من 20 شركة تأمين في السعودية." },
  ...
  return (
    <div className="overflow-hidden">
        ...
        <div className="container-x relative z-10 py-16 md:py-24">
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
            <div className="animate-slide-up space-y-6">
              ...
              <h1 className="text-balance text-4xl font-extrabold leading-tight text-white md:text-5xl lg:text-6xl">
              ...
              <div className="flex flex-wrap gap-6 pt-4">
                {[
                  ...
                  <div key={item.text} className="flex items-center gap-2 text-sm text-primary-100">
            ...
            <div className="animate-scale-in">
              <div className="rounded-3xl bg-white p-6 shadow-2xl md:p-8">
                <h3 className="mb-2 text-xl font-bold text-dark-900">احصل على عرض سعر فوري</h3>
                ...
                <form onSubmit={handleSearch} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <label className="flex cursor-pointer items-center justify-center rounded-xl border-2 border-primary-200 bg-primary-50 px-4 py-3 text-sm font-semibold text-primary-700 transition-all hover:border-primary-400">
                        <input type="radio" name="insuranceType" value="شامل" defaultChecked className="sr-only" />
                        تأمين شامل
                      </label>
                      <label className="flex cursor-pointer items-center justify-center rounded-xl border-2 border-dark-200 px-4 py-3 text-sm font-semibold text-dark-600 transition-all hover:border-primary-300">
                        <input type="radio" name="insuranceType" value="ضد الغير" className="sr-only" />
                        ضد الغير
                      </label>
                    </div>
                  </div>
                  ...
                  <p className="text-center text-xs text-dark-400">مقارنة مجانية · بدون التزام · نتائج فورية</p>
                </form>
              </div>
            </div>
          </div>
        ...
        <div className="container-x">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {insuranceTypes.map((type) => {
              ...
              return (
                <Link key={type.id} to="/insurance/$type" params={{ type: type.id }} className="group card hover:scale-[1.02]">
      ...
      <section className="section-padding bg-white">
        <div className="container-x">
          <div className="mb-12 text-center">
            <h2 className="text-3xl font-extrabold text-dark-900 md:text-4xl">ليش بيكير خيارك الأول في التأمين؟</h2>
            <p className="mt-3 text-lg text-dark-500">عندنا فريق يراقب كل صغيرة وكبيرة في السوق ويضمن أن سعرك الأقل</p>
          </div>
      ...
      <section className="section-padding bg-dark-50">
        <div className="container-x">
          <div className="mb-12 text-center">
            <h2 className="text-3xl font-extrabold text-dark-900 md:text-4xl">شركاؤنا من شركات التأمين</h2>
            <p className="mt-3 text-lg text-dark-500">نقارن لك عروض أكثر من 20 شركة تأمين</p>
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {insuranceCompanies.map((company) => (
              <div key={company.id} className="group flex h-24 items-center justify-center rounded-2xl bg-white p-4 shadow-sm ring-1 ring-dark-200/60 transition-all hover:scale-105 hover:shadow-lg">
                <div className="flex flex-col items-center gap-2">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl text-white font-bold" style={{ backgroundColor: company.color }}>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {[
              ...
              <div key={step.num} className="relative text-center">
                {idx < 2 && <div className="absolute top-12 right-0 hidden h-0.5 w-full bg-gradient-to-l from-primary-300 to-transparent md:block" />}
              ...
              <div key={t.name} className="card">
                <div className="flex gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-5 w-5 ${i < t.rating ? "fill-accent-400 text-accent-400" : "text-dark-300"}`} />
                  ))}
                </div>
